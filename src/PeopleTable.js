...
const PeopleTable = () => {
  ...
  const [sortFields, setSortFields] = useState([
    { column: "UserName", order: "Ascending" },
  ]);
  ...
  const hasCustomSort = !(sortFields.length === 1 && 
    sortFields[0].column === "UserName" && 
    sortFields[0].order === "Ascending");
  ...
  const fetchPeople = async () => {
    try {
      ...
      sortFields.forEach((field) => {
        if (field.column) {
          orderByParams.push(
            `${field.column} ${field.order === "Ascending" ? "asc" : "desc"}`
          );
        }
      });

      const orderBy =
        orderByParams.length > 0 ? orderByParams.join(",") : "UserName asc";
      ...
      const filterQuery =
        filterParams.length > 0 ? filterParams.join(" and ") : "";
  ...
  const handleRemoveSort = () => {
    setSortFields([{ column: "UserName", order: "Ascending" }]);
    setCurrentPage(1);
  };
  ...
  return (
    <div>
      ...
      <div className="people-table-container">
        ...
        {(hasAppliedFilters || hasCustomSort) && (
        ...
        <div className="action-bar">
          <div
            className={`action-item ${hasCustomSort ? 'active' : ''}`}
            onClick={() => setIsSortDialogOpen(true)}
          >
            <img src={sort} alt="Sort" className="action-icon" />
            Sort
          </div>
          <div
            className={`action-item ${hasAppliedFilters ? 'active' : ''}`}
            onClick={() => setIsFilterDialogOpen(true)}
          >
            <img src={filter} alt="Filter" className="action-icon" />
            Filter
          </div>
          <div className="action-item" onClick={fetchPeople}>
            <img
              src={refresh}
              alt="Refresh"
